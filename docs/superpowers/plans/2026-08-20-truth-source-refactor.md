...
If a ticker fails on the forced-expiry assertion, the chain genuinely lacks that
expiry — investigate rather than relaxing it. If it fails on the implied-move tolerance, the ATM selection diverged — compare `o_call["strike"]` against the merged path's chosen strike before changing the tolerance. **Do not widen the tolerance, and do not lower the 0.08 sentinel floor, to make the
